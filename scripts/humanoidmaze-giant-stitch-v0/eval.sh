#!/bin/bash
# Evaluate CD / ECD / CDGS on humanoidmaze-giant-stitch-v0.  Usage:  bash eval.sh <cd|ecd|cdgs> <seed>
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"; cd "$ROOT"
export PYTHONPATH="$ROOT:${PYTHONPATH:-}" MUJOCO_GL=egl PYOPENGL_PLATFORM=egl MPLCONFIGDIR=/tmp/matplotlib
METHOD="${1:?usage: bash eval.sh <cd|ecd|cdgs> <seed>}"; SEED="${2:-0}"; ENV=humanoidmaze-giant-stitch-v0

COMMON=(--env "$ENV" --planner_name planner --planner_epoch latest --invdyn_name invdyn --invdyn_epoch latest --seed "$SEED" \
        --ev_n_comp 11 --n_max_steps 8000 --repl_max_n 10 \
...
case "$METHOD" in
  ...
  ecd)   # ECD (Ours): bridge-energy (reaction off), map-free overlap ranking
    python -m ecd.eval "${COMMON[@]}" --eval_name ecd --ev_cp_infer_t_type ecd_chunk \
      --ecd_base_scale 0.05 --ecd_react_scale 0.0 --ecd_react_clip 0.5 --ecd_chunk_react_type none --ecd_markov_type laplacian --ecd_markov_rho 0.25 --ecd_rank_type overlap ;;
