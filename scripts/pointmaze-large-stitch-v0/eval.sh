#!/bin/bash
# Evaluate CD / ECD / CDGS on pointmaze-large-stitch-v0.  Usage:  bash eval.sh <cd|ecd|cdgs> <seed>
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"; cd "$ROOT"
export PYTHONPATH="$ROOT:${PYTHONPATH:-}" MUJOCO_GL=egl PYOPENGL_PLATFORM=egl MPLCONFIGDIR=/tmp/matplotlib
METHOD="${1:?usage: bash eval.sh <cd|ecd|cdgs> <seed>}"; SEED="${2:-0}"; ENV=pointmaze-large-stitch-v0

COMMON=(--env "$ENV" --planner_name planner --planner_epoch latest --seed "$SEED" \
        --ev_n_comp 6 --n_max_steps 1000 --repl_max_n 1 \
