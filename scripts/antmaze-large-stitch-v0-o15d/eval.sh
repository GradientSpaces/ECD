#!/bin/bash
# Evaluate CD / ECD / CDGS on antmaze-large-stitch-v0-o15d.  Usage:  bash eval.sh <cd|ecd|cdgs> <seed>
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"; cd "$ROOT"
export PYTHONPATH="$ROOT:${PYTHONPATH:-}" MUJOCO_GL=egl PYOPENGL_PLATFORM=egl MPLCONFIGDIR=/tmp/matplotlib
METHOD="${1:?usage: bash eval.sh <cd|ecd|cdgs> <seed>}"; SEED="${2:-0}"; ENV=antmaze-large-stitch-v0-o15d

COMMON=(--env "$ENV" --planner_name planner --planner_epoch latest --invdyn_name invdyn --invdyn_epoch latest --seed "$SEED" \
        --ev_n_comp 6 --n_max_steps 10000 --repl_max_n 10 \
...
case "$METHOD" in
  ...
  ecd)   # ECD (Ours): fitted-Gaussian reaction (15D), map-free overlap ranking
    python -m ecd.eval "${COMMON[@]}" --eval_name ecd --ev_cp_infer_t_type ecd_chunk \
      --ecd_base_scale 1.0 --ecd_react_scale 0.02 --ecd_react_clip 1.0 --ecd_chunk_react_type markov --ecd_markov_type fitted_gaussian --ecd_markov_rho 0.25 --ecd_rank_type overlap --ecd_prior_path "logs/$ENV/ecd_prior/gaussian_markov.pt" ;;
