#!/bin/bash
# Evaluate CD / ECD / CDGS on pointmaze-medium-stitch-v0.  Usage:  bash eval.sh <cd|ecd|cdgs> <seed>
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"; cd "$ROOT"
export PYTHONPATH="$ROOT:${PYTHONPATH:-}" MUJOCO_GL=egl PYOPENGL_PLATFORM=egl MPLCONFIGDIR=/tmp/matplotlib
METHOD="${1:?usage: bash eval.sh <cd|ecd|cdgs> <seed>}"; SEED="${2:-0}"; ENV=pointmaze-medium-stitch-v0

COMMON=(--env "$ENV" --planner_name planner --planner_epoch latest --seed "$SEED" \
        --ev_n_comp 3 --n_max_steps 1000 --repl_max_n 1 \
...
case "$METHOD" in
  ...
  ecd)   # ECD (Ours): laplacian reaction, map-free overlap ranking
    python -m ecd.eval "${COMMON[@]}" --eval_name ecd --ev_cp_infer_t_type ecd_chunk \
      --ecd_base_scale 0.03 --ecd_react_scale 0.05 --ecd_react_clip 1.0 --ecd_chunk_react_type markov --ecd_markov_type laplacian --ecd_markov_rho 0.25 --ecd_rank_type overlap ;;
